...
/**
 * @file
 * Contains \Drupal\exposed_sort_block\Plugin\Derivative\ExposedSortBlock.
 */

namespace Drupal\exposed_sort_block\Plugin\Derivative;
...
/**
 * Provides block plugin definitions for all Views Exposed Sort filters.
 *
 * @see \Drupal\views\Plugin\block\block\ViewsExposedFilterBlock
 */
class ExposedSortBlock implements ContainerDeriverInterface {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    foreach ($this->viewStorage->loadMultiple() as $view) {
      ...
      foreach ($executable->displayHandlers as $display) {
        // @Todo retrieve the correct option here
        if (isset($display) && $display->getOption('exposed_block')) {
          ...
          if ($display->usesExposedFormInBlock()) {
            ...
            $desc = t('Exposed sort form: @view-@display_id', array('@view' => $view->id(), '@display_id' => $display->display['id']));
